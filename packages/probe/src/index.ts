/* eslint-disable unicorn/no-process-exit, unicorn/prefer-top-level-await -- CLI entry point */
...
import { DEFAULT_REGISTRY_URL, fetchRegistry } from './registry'
...
const SETTLE_MS = Number(process.env.ACP_SETTLE_MS ?? 2500)
...
async function main(): Promise<void> {
  ...
  if (SKIP_CACHED) {
    log(`ACP_SKIP_CACHED=1 → cached agents will be loaded from ${CACHE_DIR}/ without probing`)
  }
